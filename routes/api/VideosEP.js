/**
 * @Date:   2020-08-31T20:58:56-05:00
 * @Last modified time: 2020-10-28T08:20:57-06:00
 */
 ...
 router.get("/getAllByFilter/:skip",
    async (req, res) => {
      try {
        ...
        if(req.query.name){
          ...
          var expression = new RegExp(``+"^"+req.query.name + "$","i")
          filter.name = {
             $in: [ expression ]
          }
          ...
          // var expression = new RegExp(``+""+req.query.nickname,"i")
          var expression = new RegExp(``+"^"+req.query.nickname + "$","i")
        ...
        if(videos && videos.length>0){
          ...
          console.log("default")
          var videoDefault = await serviceGenerics.findOne(Config, {key:"DEFAULT_VIDEO"})
 ...
 router.get("/:videoId",
    ...
    async (req, res) => {
      try {
        var video = await serviceGenerics.read(Videos, req.params.videoId)
        return res.json({
          status:status.SUCCESS,
          result:{
            video
...
router.post("/",
 async(req, res) => {
  try {
    var  video = await serviceGenerics.create(
      "Videos", req.body)
 ...
 async(req, res) => {
   try {
       ...
       var video = await serviceGenerics.patch(
         Videos, attributes, req.params.videoId
       )
